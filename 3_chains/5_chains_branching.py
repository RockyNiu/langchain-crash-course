...
branches = RunnableBranch(
    (
        lambda x: 'positive' in x if isinstance(x, str) else False,
        positive_feedback_template
        | model
        | StrOutputParser(),  # Positive feedback chain
    ),
    (
        lambda x: 'negative' in x if isinstance(x, str) else False,
        negative_feedback_template
        | model
        | StrOutputParser(),  # Negative feedback chain
    ),
    (
        lambda x: 'neutral' in x if isinstance(x, str) else False,
